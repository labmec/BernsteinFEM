...
int main ()
{
    int polynomialOrder = 4;        // basis polynomial order
    int nIntegrationPoints = 2 * polynomialOrder; // number of integration points

    TPZFMatrix<REAL> vertices({0.0, 1.0}); // vertices of the element in question (default = {0.0, 1.0})
    ...
    TPZVec<REAL> functionEvaluated(integrationPoints.Rows()); // creates the vector which will hold the values for the evaluation of the function in the integration points

    // evaluates the function in the integration points and store it in 'functionEvaluated'
    for(unsigned i = 0; i < integrationPoints.Rows(); i++)
    {
